...
class GlobalAndMainQueueViewController: UIViewController {

    private let mainLabel: UILabel = {
        let label = UILabel()
        label.text = "DispatchQueue.main"
    ...
    private let globalLabel: UILabel = {
        let label = UILabel()
        label.text = "DispatchQueue.global"
        label.font = UIFont.boldSystemFont(ofSize: 18)
        label.textColor = .white
        return label
    }()
    ...
    private let downloadButtonTwo: UIButton = {
        let button = UIButton(type: .system)
        button.setTitle("Download Image (Global)", for: .normal)
        button.setTitleColor(.blue, for: .normal)
        button.translatesAutoresizingMaskIntoConstraints = false
        return button
    }()
...
import SwiftUI
#Preview {
    GlobalAndMainQueueViewController()
}

/*
 DispatchQueue.main დროს ხდება მტავარ ნაკადზე გაშვება პროცესის, რა დროსაც ხებდა დაბლოკვა სხვა დანარცენი პროცესებიდ ა სანამ არ მორცება ერთი პროცესი მანდმადე არ მოხდება შედმეგის გაშვება


DispatchQueue.global()  დროს არ ხდება მტავარი ნაკადის დაბლოკვა ანუ პარალელრუად ეშვება რამდენიმე პროცესი , ასევე შეგვიძია მივანიჭ`თ პრიორიტეტები
qos - Quality of Service (QoS)
DispatchQueue.global(qos: .userInteractive)  ,  ყველაზე მაღალი პრიორიტეტი ,
DispatchQueue.global(qos: .userInitiated), მაღალი პრიორიტეტი ,
DispatchQueue.global(qos: .default) , სტანდატრული ტასკები
DispatchQueue.global(qos: .utility) , დაბალი
DispatchQueue.global(qos: .background) , ყველაზე დაბალი
*/
